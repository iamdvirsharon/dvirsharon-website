...
const Navbar1 = () => {
  ...
  return <div className="flex justify-center w-full py-6 px-4">
      <div className="flex items-center justify-between px-6 py-3 rounded-full shadow-lg w-full max-w-3xl relative z-10 bg-neutral-950">
        <div className="flex items-center">
          <motion.div className="w-8 h-8 mr-6" initial={{
          scale: 0.8
        }} animate={{
          scale: 1
        }} whileHover={{
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          scale: 1.05
              ...
              <a href={item.url} className="text-sm text-white hover:text-gray-600 transition-colors font-medium">
                {item.name}
              </a>
            </motion.div>)}
        </nav>
      ...
      {/* Mobile Menu Overlay */}
      <AnimatePresence>
        {isOpen && <motion.div className="fixed inset-0 bg-white z-50 pt-24 px-6 md:hidden" initial={{
        opacity: 0,
        x: "100%"
      }} animate={{
        opacity: 1,
        ...
        stiffness: 300
          ...
          delay: 0.2
            ...
            x: 20
                  ...
                  <a href={item.url} className="text-base text-gray-900 font-medium" onClick={toggleMenu}>
                    {item.name}
                  </a>
                </motion.div>)}
